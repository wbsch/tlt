...
export async function readReachableFraction(page: Page): Promise<ReachableFraction> {
  await ensureStatsExpanded(page)
  const reachableText = await readText(page.getByTestId('stats-reachable-value'))
...
export async function readTrackerStats(page: Page): Promise<TrackerStats> {
  ...
  const checked = parseInteger(await readText(page.getByTestId('stats-checked-value')), 'checked stat')
  const remaining = parseInteger(await readText(page.getByTestId('stats-remaining-value')), 'remaining stat')
...
export async function waitForAllReachable(page: Page, timeout = 20_000): Promise<ReachableFraction> {
  await ensureStatsExpanded(page)
...
export async function waitForBoot(page: Page): Promise<void> {
  await expect(page.getByRole('heading', { name: 'The Last Tracker' })).toBeVisible()
