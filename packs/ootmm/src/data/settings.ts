...
const resolveCategoryLabel = (rawCategory?: string): string => {
  ...
  const labeled = SUBCATEGORY_LABELS.get(rawCategory) ?? CATEGORY_LABELS.get(rawCategory)
...
const normalizeMultiSelectDefault = (value: unknown): { type: 'all' | 'none' | 'specific'; values?: unknown[] } => {
  if (value && typeof value === 'object' && 'type' in value) {
    return deepCloneDefault(value) as { type: 'all' | 'none' | 'specific'; values?: unknown[] }
  }
...
const applyDefaults = (definitions: SettingDefinition[]): SettingDefinition[] =>
  definitions.map((def) => {
    if (!Object.prototype.hasOwnProperty.call(DEFAULT_OOTMM_SETTINGS, def.key)) {
      console.warn(`[OoTMM Settings] Missing default for ${def.key} in DEFAULT_OOTMM_SETTINGS`)
      return def
    }
    const rawDefault = (DEFAULT_OOTMM_SETTINGS as Record<string, unknown>)[def.key]
    const normalizedDefault =
      def.type === 'multi-select' ? normalizeMultiSelectDefault(rawDefault) : deepCloneDefault(rawDefault)
...
export const ALL_SETTINGS_DEFINITIONS = applyDefaults(BASE_SETTINGS_DEFINITIONS)
export const SETTINGS_DEFINITIONS = applyWhitelist(ALL_SETTINGS_DEFINITIONS)
export const TRACKER_DEFAULT_SETTINGS = buildDefaultsRecord(ALL_SETTINGS_DEFINITIONS)
