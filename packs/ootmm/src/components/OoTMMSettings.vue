...
import {
  SPECIAL_CONDS,
  SPECIAL_CONDS_FIELDS,
} from '@ootmm/core/settings/index';
...
function getSpecialCondName(condKey: string) {
  return (SPECIAL_CONDS as Record<string, SpecialCondDef>)[condKey]?.name;
}
...
function computeSpecialCondMax(condState: Record<string, unknown>) {
  ...
  for (const [fieldKey, fieldDef] of specialFields) {
    ...
    const raw =
      typeof fieldMax === 'function'
        ? (fieldMax as (settings: Record<string, unknown>) => unknown)(settings)
        : fieldMax;
  ...
  if (condState.masksOot && condState.masksRegular) {
    const sharedMaskCount = Object.keys(settings)
      ...
      .filter((key) => Boolean(settings[key as keyof typeof settings])).length;
    max -= sharedMaskCount;
  }
...
const visibleSettings = computed(() =>
  SETTINGS_DEFINITIONS.filter(isSettingVisible),
);
...
const settingsByCategory = computed(() => {
  return filteredSettings.value.reduce(
    (acc, setting) => {
...
const specialCondKeysBySetting = computed(() => {
  ...
  for (const setting of filteredSettings.value) {
    ...
    const withMoon =
      setting.key === moonAnchorKey.value ? ['MOON', ...base] : base;
...
<template>
  <div class="settings-panel">
    ...
    <div class="settings-content">
      <div
        v-for="(categorySettings, category) in settingsByCategory"
        :key="category"
          ...
          >
            <div class="setting-item">
              ...
              >
                ...
                <div
                  v-if="getMultiSelectMode(setting.key) === 'specific'"
                  class="setting-multiselect-options"
                >
                  <label
                    v-for="opt in getVisibleOptions(setting)"
                    :key="optionKey(opt.value)"
            ...
            <div
              v-if="specialCondKeysBySetting[setting.key]?.length"
              class="setting-special"
            >
              <div class="setting-special-label">Special Condition</div>
              <div class="special-grid">
                <div
                  v-for="condKey in specialCondKeysBySetting[setting.key]"
                  :key="condKey"
                  class="special-card"
                >
                  <div class="special-header">
                    <div class="special-title">
                      {{ getSpecialCondName(condKey) }}
                    </div>
                    <button
                      type="button"
                  ...
                  <div
                    v-if="!isSpecialCondExpanded(condKey)"
                    class="special-summary"
                  >
                    <div class="special-summary-text">
                      {{ getSpecialCondSummary(condKey) }}
                    </div>
                    <div class="special-summary-count">
                      Amount: {{ getSpecialCondCount(condKey) }} /
                      {{ getSpecialCondMax(condKey) }}
                    </div>
                  </div>
                  ...
                  <div v-else class="special-fields">
                    ...
                    >
                      <span class="special-label">{{
                        (fieldDef as any).name
                      }}</span>
                      <input
                        type="checkbox"
                        class="special-toggle"
                        :checked="
                          Boolean(getSpecialCondState(condKey)[fieldKey])
                        "
                        @change="toggleSpecialCondField(condKey, fieldKey)"
                      />
                    </label>
                    <label class="special-count">
                      <span class="special-count-label"
                        >Amount (max: {{ getSpecialCondMax(condKey) }})</span
                      >
                      <input
                        type="number"
                        min="0"
      ...
      <div v-if="fallbackSpecialCondKeys.length > 0" class="settings-fallback">
        ...
        <div class="setting-special">
          <div class="special-grid">
            <div
              v-for="condKey in fallbackSpecialCondKeys"
              :key="condKey"
              class="special-card"
            >
              <div class="special-header">
