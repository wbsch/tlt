...
const FALLBACK_ICON = ICON_SET.has('collectible')
  ? 'collectible'
  : MAP_ICON_INDEX[0];
...
function parseCliArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (arg === '--marker-center-offset') {
      throw new Error(
        `Missing value for --marker-center-offset.\n${usageMessage()}`,
      );
    }
...
function extractZipEntry(zipBuffer: Buffer, entry: ZipEntry): Buffer {
  ...
  if (localHeaderOffset + 30 > zipBuffer.length) {
    throw new Error(
      `Local file header exceeds archive length for ${entry.name}`,
    );
  }

  if (
    zipBuffer.readUInt32LE(localHeaderOffset) !== LOCAL_FILE_HEADER_SIGNATURE
  ) {
  ...
  return (
    entries.find((entry) => {
      ...
      return (
        ...
        normalizedName.toLowerCase() === 'stack.xml'
      );
    }) ?? null
  );
}
...
function parseXmlTags(xml: string): XmlTag[] {
  ...
  for (const match of xml.matchAll(re)) {
    ...
    const selfClosing =
      !closing && (match[4] === '/' || /\/\s*$/.test(attrsRaw));
...
function parseImageSize(xml: string): { width: number; height: number } {
  ...
  if (!Number.isFinite(width) || !Number.isFinite(height)) {
    throw new Error(
      `Invalid root image size attributes: w=${attrs.w ?? '?'} h=${attrs.h ?? '?'}`,
    );
  }
...
function guessIcon(layer: LayerEntry): string {
  const context = normalizeText(
    [...layer.path, layer.layerName, layer.src].join(' '),
  );
  ...
  for (const icon of MAP_ICON_INDEX) {
    ...
    const overlap = iconTokens.reduce(
      (count, token) => count + Number(contextTokens.has(token)),
      0,
    );
...
function toTitleCaseWord(word: string, index: number): string {
  ...
  if (
    index === 0 &&
    ['oot', 'mm', 'ootmm'].includes(normalized.toLowerCase())
  ) {
...
async function generate(): Promise<void> {
  ...
  const outputJsonPath = path.resolve(
    `packs/ootmm/src/data/maps/${mapBaseName}.json`,
  );
  ...
  if (!stackXmlEntry) {
    throw new Error(
      `Could not find root-level stack.xml in ${path.basename(inputOra)}`,
    );
  }
  ...
  if (!largestPngEntry) {
    throw new Error(
      `Could not find any .png files in ${path.basename(inputOra)}`,
    );
  }
